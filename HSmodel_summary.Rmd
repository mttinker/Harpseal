---
params:
 rep_title: "Harp Seal Population Model" 
...
knitr::opts_chunk$set(fig.width=12, fig.height=8, fig.align = "left") 
```
...
required_Packages<-c("knitr","readxl", "gtools", "stats", "dplyr", "ggplot2", "fitdistrplus",
                     "bayesplot","lattice","coda","kableExtra","gridExtra","cowplot")
...
and where $\beta$, $\phi_F$, $\theta_F$, $\delta_F$, and $\sigma_F$ are all parameters to be estimated. The direciton and magnitude of effects of the climate index (CI) were dtermined by parameter $\delta_F$, and we assume a 1 year lag (i.e. female fecundity is primarily affected by the previous years values). Equation 2.1 also includes density-dependent effects (determined by current population abundance and parameters $\phi_F$ and $\theta_F$) and is parameterized such that fecundity is asymptotic by age 8 (annual fecundity for females aged >8 yrs is set equal to *F*(8,*t*).  
...
$$ 
\begin{equation}
{\gamma_A}(i) =  {\alpha_0} - {\alpha_1} \cdot ({i}-10) + {\alpha_2} \cdot ({i}-10)^2 (\#eq:f3)
\end{equation}
$$

where ${\alpha_0}$, ${\alpha_1}$ and ${\alpha_2}$ are parameters to be estimated. Note that we re-center the age vector by subtracting 10 to simplify fitting and parameter interpretation: specifically, ${\alpha_0}$ represents baseline log hazards for a 10 yr-old adult, ${\alpha_1}$ represent additional log hazards for younger animals and ${\alpha_2}$ represents additional log hazards for older animals. For juveniles (${\gamma_0}$) we solve the same equation with *i* = 0. 

We also allow for effects of density-dependence and, for juveniles, environmental variation as captured by the climate index (*CI*) and additional stochasticity. Density-dependent log-hazards were calculated as a function of current population abundance (similar in form to density-dependent effects on fecundity):
$$
\begin{equation}
{\gamma_D} = {\left( {{\phi _S} \cdot {N’}(t)} \right)}^{\theta_S} (\#eq:f4)
\end{equation}
$$

while climate index effects were determined by parameter $\delta_S$ and environmental stochasticity in survival (${\varepsilon _S}(t)$) was modeled as a random hierarchical effect:
$$
\begin{equation}
{\varepsilon _S}(t)\sim normal\left( {0,{\sigma _S}} \right) (\#eq:f5)
\end{equation}
$$ 

where $\phi_S$, $\theta_S$ $\delta_S$ and $\sigma_S$ are all parameters to be estimated. We then combine these log-hazard effects and transform by exponentiation to derive the baseline hazard terms for juveniles and adults: 
$$
\begin{equation}
{h_0}(t) = \exp \left({\omega + {\gamma _0} + {\gamma_D} + \delta_S  \cdot CI(t) + {\varepsilon _S}(t)} \right) (\#eq:f6)
\end{equation}
$$

$$
\begin{equation}
{h_A}(i,t) = \exp \left({\omega + {\gamma_A}(i) + \left( {1 \over i+0.5} \right)^{\zeta} \cdot \gamma_D} \right) (\#eq:f7)
\end{equation}
$$

where $\omega$ is a nuisance constant (arbitrarily fixed at -7) that improves fitting and simplifies interpretation by allowing all other log-hazard parameters to take on values between 0 and 5. We note that for adults, variation due to climate index and stochastic effects is assumed to be minor and thus safely omitted, while density-dependent effects are assumed to be small relative to juveniles and age-dependent, with estimated parameter $\zeta$ controlling the rate at which density-dependent effects decline with age. 
...
$$
\begin{equation}
IC(a,t) = (\% icecov(a,t) - avg.[ \% icecov(a)] ) / avg.[ \% icecov(a)] (\#eq:f8)
\end{equation}
$$
...
$$
\begin{equation}
{\gamma_{IC}}(a,t) = ({-\omega}+1) \cdot inv.logit[{\psi_1} - {\psi_2} \cdot IC(a,t)] (\#eq:f9)
\end{equation}
$$

where $\psi_1$ and $\psi_2$ are estimated parameters. Recognizing that ice anomalies in the Gulf (a = 1 or 2) and at the Front (a = 3) might have differing effects on juvenile survival, we fit separate values of parameter $\psi_1$ for the Gulf ($\psi_{1G}$) and Front ($\psi_{1F}$). To calculate the population-level hazards from ice anomalies, we transform by exponentiation and then take the weighted average across the 3 breeding areas (*a* = 1 for S. Gulf, 2 for N. Gulf, 3 for Front), with weighting determined by the proportion of pups born in each breeding area (*P*(*a,t*)):
$$
\begin{equation}
{h_{IC}}(t) = \sum\limits_{a = 1}^3 {P(a,t) \cdot } \exp \left( {{\omega} + {\gamma_{IC}}(a,t)} \right) (\#eq:f10)
\end{equation}
$$ 
...
$$
\begin{equation}
{\gamma _{H0}}(t)\sim normal\left( {{{\bar \gamma }_{H0}},{\sigma _{H}}} \right) (\#eq:f11)
\end{equation}
$$
...
$$
\begin{equation}
{h_{H0}}(t) = \exp \left( {{\omega} + {\gamma _{H0}}(t)} \right) (\#eq:f12)
\end{equation}
$$

$$
\begin{equation}
{h_{HA}}(t) = \exp \left( {{\omega} + {\gamma _{HA}}(t)} \right) (\#eq:f13)
\end{equation}
$$
...
$$
\begin{equation}
{S_0}(t) = \exp \left( { - 1 \cdot \left[ {{h_0}(t) + {h_{IC}}(t) + {h_{H0}}(t) } \right]} \right) (\#eq:f14)
\end{equation}
$$

$$
\begin{equation}
{S_A}(i,t) = \exp \left( { - 1 \cdot \left[ {{h_A}(i,t) + {h_{HA}}(t)} \right]} \right) (\#eq:f15)
\end{equation}
$$

We also include a constant term for newborn pup survival ($S_{np}(t)$) to account for abortions and early pup mortality that occurs prior to the pup counts, thereby allowing for some decoupling of pregnancy rates and pup counts. Based on previously published analyses (Stenson et al 2014, 2016) it is estimated that the percent of pregnancies leading to viable pups averages around 95% but may vary from 90-100%. For the present model we fix $S_{np}$ at 0.95.
...
$$
\begin{equation}
n(1,t + 1) = 0.5 \cdot \left( {\sum\limits_{i = 1}^{36} {n(i,t) \cdot F(i,t)} } \right) \cdot {S_{np}}(t) \cdot {S_0}(t) (\#eq:f16)
\end{equation}
$$
...
$$
\begin{equation}
n(i,t + 1) = n(i,t) \cdot {S_A}(i,t) (\#eq:f17)
\end{equation}
$$
...
$$
\begin{equation}
n(36,t + 1) = n(35,t) \cdot {S_A}(35,t) + n(36,t) \cdot {S_A}(36,t) (\#eq:f18)
\end{equation}
$$
...
Agedist\_pred(t) = {{\left[ {\matrix{
 ...
 } } \right]} \over {\sum\limits_{i = m}^{36} {n(i,t)} }} (\#eq:f19)
\end{equation}
$$
...
$$
\begin{equation}
Pups\_pred(t) = 0.5 \cdot \left( {\sum\limits_{i = 1}^{36} {n(i,t) \cdot F(i,t)} } \right) \cdot {S_{np}}(t) (\#eq:f20)
\end{equation}
$$
...
$$
\begin{equation}
{f_{H0}}(t) = {{{h_{H0}}(t)} \over {\left[ {{h_0}(t) + {h_{IC}}(t)} + {h_{H0}}(t) \right]}} (\#eq:f21)
\end{equation}
$$
...
$$
\begin{equation}
pred\_{H_0}(t) = 0.5 \cdot \left( {\sum\limits_{i = 1}^{36} {n(i,t) \cdot F(i,t)} } \right) \cdot {S_{np}}(t) \cdot \left( {1 - {S_0}(t)} \right) \cdot {f_{H0}}(t) \cdot Q_0(t) (\#eq:f22)
\end{equation}
$$
...
$$
\begin{equation}
{f_{HA}}(i,t) = {{{h_{HA}}(t)} \over {\left[ {{h_A}(i,t) + {h_{HA}}(t)} \right]}} (\#eq:f23)
\end{equation}
$$
...
$$
\begin{equation}
pred\_{H_A}(t) = \sum\limits_{i = 1}^{36} {n(i,t) \cdot \left( {1 - {S_A}(i,t)} \right)  \cdot {f_{HA}}(i,t) \cdot Q_A(t) } (\#eq:f24)
\end{equation}
$$
...
$\delta_F$| Effect of environmental conditions (*CI* index) on fecundity
...
$\delta_S$| Effect of environmental conditions (*CI* index) on juvenile survival
$\sigma_S$| Unexplained variation in juvenile survival
$\zeta$   | Parameter to scale density-dependent effects for adult animals by age
...
${\sigma_{H}}$ | Variation in annual log hazard rates for harvest mortality
$\tau$    | Precision parameter, Dirichlet-multinomial distribution of age-class samples
...
$$
\begin{equation}
Pups(t)\sim normal\left( {Pups\_pred(t),S{E_p}(t)} \right) (\#eq:f25)
\end{equation}
$$
...
$$
\begin{equation}
NPr(i,t)\sim binomial\left( {NF(i,t),F(i,t)} \right) (\#eq:f26)
\end{equation}
$$
...
$$
\begin{equation}
\pi (t)\sim dirichlet\left( {\tau \cdot  Agedist\_pred(t)} \right) (\#eq:f27)
\end{equation}
$$
...
$$
\begin{equation}
H{_0}(t)\sim normal\left( {pred\_{H_0}(t),S{E_{HV}}} \right) (\#eq:f28)
\end{equation}
$$

$$
\begin{equation}
H{_A}(t)\sim normal\left( {pred\_{H_A}(t),S{E_{HV}}} \right) (\#eq:f29)
\end{equation}
$$
...
Estimated fecundity rates vary as a function of age: pregnancy rates are 0 up to age 3, then increase rapidly for ages>3 to an asymptotic value by age 8 (Fig. 3). Density-dependent effects also cause a gradual reduction in pregnancy rates as population abundance increases (Fig. 4). Environmental stochasticity resulted in substantial deviations from mean expected fecundity rates over time, with a occasional "extreme" deviations such as 1960 or 2010-11 (Fig. 5). The environmental condition index had a significant and positive effect on fecundity ($\delta$ in Table 3.1), accounting for some of the variation in pregnancy rates. 
...
pl7 = ggplot(data=dp7,aes(x=Year,y=Fdev)) +
  ...
  ggtitle("Figure 5. Stochastic variation in fecundity") + theme_bw()
print(pl7)
```

<br/>

As with fecundity, survival rates also vary with age (Fig. 6). Juveniles experience significantly higher levels of mortality than older animals, and their survival declines sharply during years when the ice anomaly index drops below a threshold value (Fig. 7). Juveniles (and to a lesser extend subadults) also experience density-dependent variation in survival, with an accelerating decrease in survival as abundance increases above 5 million animals (Fig. 6 and 8). And environmental stochasticity resulted in substantial deviations from mean expected hazard rates for juveniles, with a few notable periods of elevated log hazards (e.g. early 2000s; Fig. 9).
...
pl10 = ggplot(dp10,aes(x=Ice_Anomaly,y=SvIce,fill=Area)) +
  ...
  ggtitle("Figure 7. Effect of ice cover on pup survival") +
  theme_classic()
print(pl10)
```
...
Some of the variation in fecundity and juvenile survival was explained by environmental fluctuations that were captued by the climate index, *CI*. Specifically, both female pregnancy rates and juvenile survival declined as a function of *CI* (Fig. 10) 
...
pl11a = ggplot(dp11, aes(x=CI,y=Fecundity)) +
  ...
  ggtitle("Figure 10.A) Effect of climate index (1 year lag) on adult fecundity") + theme_classic()
#print(pl11a)

pl11b = ggplot(dp11, aes(x=CI,y=Survival)) +
  geom_ribbon(aes(ymin=S0_lo,ymax=S0_hi),alpha=0.3) +
  geom_line() + labs(x = "Climate index (CI)",y="Annual survival, ageclass 0") +
  ggtitle("Figure 10.B) Effect of climate index on juvenile survival") + theme_classic()
# print(pl11b)

plot_grid(pl11a,pl11b,nrow = 2)

```

<br/>

The harvest/bycatch hazards varied substantially over the time series, with the magnitude of variation measured by parameter ${\sigma _{H}}$ (Tables 2.1, 3.1). Per-capita harvest/bycatch hazard ratios for juveniles (pups and beaters) were >7 times higher on average than equivalent hazard ratios for adults. The estimated annual harvest hazard rates explained virtually all variation in the observed time series of harvest/bycatch numbers (Fig. 11). 
...
pl5 = ggplot(data=dp5,aes(x=Year,y=HVexp,group=Group,color = Group, fill=Group)) +
  ...
  geom_point(aes(y=Obs),size=2) +
  ggtitle("Figure 11. Model estimated vs observed harvest/bycatch mortality, by age class") + 
  scale_color_brewer(palette="Dark2") +
  scale_fill_brewer(palette="Dark2") +
  theme_classic()
print(pl5)
...
By integrating the various environmental and demographic effects summarized above, along with harvest/bycatch mortality, the model was able to capture most of the temporal variation in the observed data sets on adult pregnancy rates (Fig. 12) and pup abundance (Fig. 13). 
...
pl4 = ggplot(data=dp4,aes(x=Year,y=PRexp)) +
  ...
  ggtitle("Figure 12: Model estimated vs observed pregnancy rate over time",
...
pl2 = ggplot(data=dp2,aes(x=Year,y=Pexp)) +
  ...
  ggtitle("Figure 13: Model estimated vs observed pup counts")  + theme_bw()
print(pl2)
```

<br/>

Absolute abundance of the harp seal population is not directly measured by empirical data; however, the model nonetheless estimates population abundance as a latent variable. The model-estimated trends in population abundance (adults plus pups-of-the-year) are shown in Fig. 14.  
...
dp1 = dp1[-Nyrs,]
dp1 = dp1[-1,]
pl1 = ggplot(data=dp1,aes(x=Year,y=N2exp)) +
      geom_ribbon(aes(ymin=N2_lo,ymax=N2_hi),alpha=0.3) +
      geom_line() + labs(x = "Year",y="Estimated abundance") +
      ggtitle("Figure 14. Model estimated abundance (including pups)") + theme_bw()
print(pl1)
```

<br/>

As an additional posterior predictive check of model goodness-of-fit, we used Monte Carlo methods to generate "out-of-sample" hind-cast projections of population dynamics (initiated with the estimated abundance in 1951 and projected forward with random effects not fit to the data). The distribution of hind-cast projections encompassed the actual model-fit estimate of population abundance, indicating good model fit (Fig. 15). 

<br/>

```{r Fig15, echo=FALSE, warning=FALSE}
load("TAC_K_est.rdata")
titletxt = "Figure 15: Hindcast 'out-of-sample' stochastic simulations"
subtxt = "  Note: harvest, CI and IC levels as-observed, red line = model-estimated abundance) "
...
In order to estimate carrying-capacity (*K*), future projections of population dynamics were generated using Monte Carlo simulations of the model, with harvest/bycatch hazards forced to 0, and under two scenarios of future ice cover and environmental conditions. The distribution of future projections stabilized after several decades (Fig. 16), with the mean long-term abundance for each scenario corresponding to *K* in the absence of harvest/bycatch mortality (Table 3.2). 

A similar Monte Carlo simulation approach provided recommended TAC values under different thresholds and assumptions about the ratio of adults:pups harvested (Table 3.3). These TAC recommendations are based on currently-available data and model estimates of all demographic and environmental processes. 

<br/>

```{r Fig16, echo=FALSE, warning=FALSE, fig.height = 5, fig.width = 12, fig.align = "left"}
titletxt1 = "Figure 16 A): Estimted K, pre-2000 conditions"
...
plt_N_Kest2 = ggplot(data=dpN_K[ii,],
  ...
  theme_classic()
# print(plt_N_Kest1)
plot_grid(plt_N_Kest1,plt_N_Kest2,nrow = 1) #,labels = c(titletxt1,titletxt2))
...
Ktab1 = data.frame(Metric = paste0("Estimated abundance, ", max(Year)), 
                  Mean = dp1$N2exp[Nyrs-2]/1000000, 
                  SD = dp1$N_sd[Nyrs-2]/1000000, 
                  CI95_low=dp1$N2_lo[Nyrs-2]/1000000,
                  CI95_high=dp1$N2_hi[Nyrs-2]/1000000)
