...
and where $\beta$, $\phi_F$, $\delta$, and $\sigma_F$ are all parameters to be fit. The environmental index (*CE*) values were log-transformed, which both normalized values and resulted in a mean value (for 1951-2019) of approximately 0.
...
Note that ice-based hazards on a given year vary across breeding areas (*b* = 1 for S. Gulf, 2 for N. Gulf, 3 for Front), and thus ice hazard values are weighted by the proportional distribution of pups in each area (*PD*(*b,t*)). The area-specific hazards associated with ice cover are assumed to be negligible in years of high or “normal” ice cover but increase rapidly as ice cover drops to anomalously low values (Stenson and Hammill 2014).  Accordingly, the ice-associated log hazard rate ($\gamma _I$) is modeled as a non-linear function of ice cover:
$${\gamma _I}(b,t) = {\psi _1} \cdot {\left( {{1 \over {\exp \left( {IC(b,t)} \right)}}} \right)^{{\psi _2}}} - 1$$
where *IC*(*b,t*) is the ice cover anomaly index value in area *b* in year *t*, calculated as annual deviations from the average % cover values for 1969-2000, rescaled to vary from -1 to 1. In the above equations, ${\gamma _0}$ is a nuisance parameter fixed at -7, ${\gamma _J}$ is set to give a maximum juvenile survival rate (at low population density and without harvest) of 0.85 (Roff and Bowen 1983), and ${\phi _J}$, $\theta$, $\psi$, ${{\bar \gamma }_{HVJ}}$ and ${\sigma _{HV}}$ are all parameters to be fit.

#### Adult survival
The factors that determine the probability of an adult surviving from one year to the next include baseline adult hazards and harvest/bycatch mortality. Survival from these competing hazards is calculated as:
$${S_A}(t) = \exp \left( { - 1 \cdot \left[ {{h_A}(t) + {h_{HVA}}(t)} \right]} \right)$$
where the cause-specific, instantaneous hazards from natural mortality (${h_A}(t)$) and harvest/bycatch mortality (${h_{HVA}}(t)$) are calculated as:
...
In the above equations, ${\gamma _0}$ is a nuisance parameter fixed at -7, ${\gamma _A}$ is set to give a baseline adult survival rate (without harvest) of 0.95 (Roff and Bowen 1983), and ${{\bar \gamma }_{HVJ}}$ and ${\sigma _{HV}}$ are parameters to be fit.
...
The data on pregnancy rates of sampled females are treated as a binomial variable:  specifically, given a sample of *N* females of age *a* in year *t* (*NF*(*a,t*)), the number of pregnant females (*NP*(*a,t*)) is assumed to follow a binomial distribution with probability determined by the model-estimated fecundity rates:
$$NP(a,t)\sim binomial\left( {NF(a,t),F(a,t)} \right)$$
The female age composition data are treated as a multinomial variable: specifically, samples of adult females in year *t* are assorted by age into vectors of counts, *Agecounts*(*t*) = [*NF*(3,*t*),*NF*(4,*t*)… *NF*(*A,t*)], and vectors are assumed to follow a multinomial distribution with probabilities determined by the model predicted age distributions: 
...
The best-fit function for Fecundity includes a significant age effect, such that pregnancy rates decreases for ages less than 8, with the rate of decreaase accelerating for the youngest age classes (Fig. 1). Density-dependent effects also cause a non-linear, accelerating reduction in pregnancy rates as population abundance increases (Fig. 2).  
...
Juvenile survival rates show a similar density-dependent trend, with a fairly rapid decrease in survival as abundance (excluding pups) increases above 4 million animals (Fig. 3)
...
In addition to density-dependent effects, there were also effects of environmental stochasticity and ice cover anomalies on female pregnancy and pup survival. The environmental condition index was positively correlated with fecundity rates, although the 95% CI for $\delta$ encompassed zero suggesting only marginal signifance (Table 2). However, there was considerable unexplained variance in fecundity rates, represented by the parameter $\sigma_F$ (Table 1, 2), which corresponds to environmental stochasticity. There was a dramatic reduction in juvenile survival as ice cover dropped to the lowest levels (Fig. 4). 
...
The harvest/byatch hazards varied substantially over the time series, with the magnitude of variaiton measured by parameter ${\sigma _{HV}}$ (Table 1, 2). On average, the estimated per-capita harvest mortality rates for juvniles, or "beaters" (${{\bar \gamma }_{HVJ}}$), were almost double those for adults (${{\bar \gamma }_{HVA}}$ ; Table 2). The estimated annual harvest rates explained virtually all the variation in observed data on harvest/bycatch numbers (Fig. 7). 
...
While absolute abundance of the harp seal population is not directly measured by empirical data, the model nonetheless estimates population abundance as a latent variable. The model-estimated trends in population abundance are shown in Fig. 8 (excluding pups-of-the-year) and Fig. 9 (including pups-of-the-year).  
