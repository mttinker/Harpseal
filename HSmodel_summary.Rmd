...
$$
\begin{equation}
{h_0}(t) = \exp \left({\omega + {\gamma _0} + {\gamma_D} + {\varepsilon _S}(t)} \right) (\#eq:f6)
\end{equation}
$$
...
where $\omega$ is a nuisance constant (arbitrarily fixed at -7) that improves fitting and simplifies interpretation by allowing all other log-hazard parameters to take on values between 0 and 5. We note that for adults, stochastic effects are assumed to be very small and thus safely omitted, while density-dependent effects are assumed to be small relative to juveniles and age-dependent, with estimated parameter $\zeta$ controlling the rate at which density-dependent effects decline with age. 
...
ix = c(which(vns=="N0"),which(vns=="beta1"),which(vns=="beta2"),
       ...
       which(vns=="sigS"),which(vns=="zeta"),which(vns=="psi1"),
...
row.names(statsum) = c("N0(millions)","Beta_1","Beta_2","phi_F","theta_F",
                       "delta","sigma_F","alpha_0","alpha_1","alpha_2",
                       "phi_S","theta_S","sigma_S","zeta","psi_1","psi_2",
...
titletxt = "Figure 14: Hindcast stochastic simulations of abundance, observed harvest"
...
In order to estimate carrying-capacity (*K*), future projections of population dynamics were generated using Monte Carlo simulations of the model, with harvest/bycatch hazards forced to 0. The distribution of future projections stabilized after several decades (Fig. 15), with the mean long-term abundance corresponding to *K* in the absence of harvest/bycatch mortality (Table 3.2). 
