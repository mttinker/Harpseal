...
required_Packages<-c("knitr","readxl", "gtools", "stats", "dplyr", "ggplot2", "fitdistrplus",
                     "bayesplot","lattice","coda","kableExtra","gridExtra")
...
We also allow for effects of density-dependence and, for juveniles, environmental variation as captured by the climate index (*CI*) and additional stochasticity. Density-dependent log-hazards were calculated as a function of current population abundance:
$$
\begin{equation}
...
ix = c(which(vns=="N0"),which(vns=="beta1"),which(vns=="beta2"),
       ...
       which(vns=="dlta[2]"),which(vns=="sigS"),which(vns=="zeta"),which(vns=="psi1"),
...
row.names(statsum) = c("N0(millions)","Beta_1","Beta_2","phi_F","theta_F",
                       ...
                       "phi_S","theta_S","delta_S","sigma_S","zeta","psi_1","psi_2",
...
pl11a = ggplot(dp11, aes(x=CI,y=Fecundity)) +
  ...
  ggtitle("Figure 10.A) Effect of climate index on adult fecundity") + theme_classic()
#print(pl11a)
...
In order to estimate carrying-capacity (*K*), future projections of population dynamics were generated using Monte Carlo simulations of the model, with harvest/bycatch hazards forced to 0. The distribution of future projections stabilized after several decades (Fig. 16), with the mean long-term abundance corresponding to *K* in the absence of harvest/bycatch mortality (Table 3.2). 
...
```{r Fig16, echo=FALSE, warning=FALSE}
titletxt = "Figure 16: Model projected abundance with no human-caused mortality"
subtxt =  paste0("Estimated long-term equilibrium (K) = ", format(Ktab$Mean,digits = 3),
                 "million (CI95 = ",format(Ktab$CI95_low,digits = 3),
                 " - ", format(Ktab$CI95_high, digits = 3),")")

plt_N_Kest = ggplot(data=dpN_K,aes(x=Year,y=N_pred_mean)) +
  geom_ribbon(aes(ymin=N_pred_lo,ymax=N_pred_hi),alpha=0.3) +
  geom_line() + labs(x = "Year",y="Projected abundance (millions)") +
  geom_hline(yintercept=Ktab$Mean, linetype="dashed", color = "red") +
  ggtitle(titletxt,subtitle =subtxt) + theme_classic()
print(plt_N_Kest)
