...
tmp$Haz_lo = sumstats[which(startsWith(vns,"haz_Ice[2")),5]
tmp$Haz_hi = sumstats[which(startsWith(vns,"haz_Ice[2")),7]
...
pl10 = ggplot(dp10,aes(x=Ice_Anomaly,y=SvIce)) +
  geom_ribbon(aes(ymin=SvIce_lo,ymax=SvIce_hi,fill=Area),alpha=0.25) +
  geom_line(aes(color=Area)) +
  geom_vline(xintercept = 0) +
  geom_line(data=Ice_effect_pri,aes(x=Ice_cover,y=LoPrior),color="darkgrey",linetype="dashed") +
  geom_line(data=Ice_effect_pri,aes(x=Ice_cover,y=HiPrior),color="darkgrey",linetype="dashed") +
...
psi1 = sumstats[startsWith(vns,"psi1[")==T,1]
psi2 = sumstats[which(vn=="psi2"),1]
...
haz_base = exp(omega + gamma_0 + gamma_D + dlta2 * CIadj + epsS ) 
...
haz_IC = numeric(length = length(haz_base))
for (i in 1:length(haz_base)){
  haz_IC[i] = PA[i,1] * exp(omega + 8 * inv.logit(psi1[1] - psi2*IC[i,1])) +
  PA[i,2] * exp(omega + 8 * inv.logit(psi1[1] - psi2*IC[i,2])) +
  PA[i,3] * exp(omega + 8 * inv.logit(psi1[2] - psi2*IC[i,3])) 
}
haz_tot = haz_DD + haz_CI + haz_IC + haz_H0
Mort = 1 - exp(-haz_tot)
Mort_DDexp = 1 - exp(-haz_DD_exp)
Mort_DDexp = Mort_DDexp * (max(Mort)/max(Mort_DDexp)) * .6

dp14 = data.frame(Year=Year,
                  Density_stochastic = Mort * haz_DD/haz_tot,
                  Climate_effects = Mort * haz_CI/haz_tot,
                  Ice_effects = Mort * haz_IC/haz_tot,
...
dp14$hazard_type = factor(dp14$hazard_type,
                          levels = c("Removals","Ice_effects",
                                     "Climate_effects","Density_stochastic"))

pl14 = ggplot(dp14, aes(x=Year)) + 
  geom_area(aes(y=Mortality, fill=hazard_type), alpha=0.5) + 
  geom_line(data=data.frame(Year=Year, Mortality = Mort_DDexp, hazard_type="Baseline + D-D, no stochasticity"),
  ...
  scale_fill_brewer(type="qual",palette = "Set1", name="Hazard type",
                    labels=c("Removals","Ice anomalies",
                            "Climate effects","Baseline + D-D")) +
  scale_color_manual(values = c("purple"),name=NULL) + 
  labs(x="Year",y="Mortality rate") +
  scale_x_continuous(breaks = seq(1950,2020,by=5)) +
  theme_bw() + theme(legend.position = c(0.25, 0.9),
                     panel.grid.minor.y = element_blank(),
                     panel.grid.minor.x = element_blank())
print(pl14)
#
...
ix = c(which(vns=="N0"),which(vns=="beta1"),which(vns=="beta2"),
       ...
       which(vns=="dlta[2]"),which(vns=="sigS"),which(vns=="zeta"),which(vns=="psi1[1]"),
       which(vns=="psi1[2]"),which(vns=="psi2"),which(vns=="gamma_H0_mn"),
...
row.names(statsum) = c("N0(millions)","Beta_1","Beta_2","phi_F","theta_F",
                       ...
                       "phi_S","theta_S","delta_S","sigma_S","zeta","psi_1_G",
                       "psi_1_F","psi_2","Gamma_H0_bar","Gamma_HA_bar",
                       "sigma_H","tau")
...
Npri = length(ix)
df.pri = read_excel("./data/Priordefs_v1.xlsx")
Paramname = character()
Priorvals = numeric()
Priorprob = numeric() 
Postdens = numeric()  
