...
data {
  ...
  real Age_bias_adj ;             // age bias: mean proportional reduction in sampled younger ages
}
//
transformed data{
  //real tau = 1 ;
  vector[NCages] Ageadj = rep_vector(1, NCages) ;
  vector[Nyrs-1] WI_CI ;
  if(NCage1 < 8){
    Ageadj[1 : (8 - NCage1)] = rep_vector((1-Age_bias_adj/100), (8 - NCage1)) ;
  }
  for (i in 1:(Nyrs-1)){
...
transformed parameters {
  ...
  for (i in 1:(Nyrs-1)){
    ...
    Avc[i] = (Ageadj .* n[i][NCage1:Nages]) / sum((Ageadj .* n[i][NCage1:Nages])) ;
