...
NCage1 = 4; # Note: recommend 8 or 9, as younger ages subject to negative sampling bias
NCyear1 = 1990 ; # First year to include age class data
nburnin = 300
nsamples = 2500
# End user parameters--------------------------------------------------------
#
...
NCobs = nrow(AgeComp)
Agects = AgeComp; Agects = Agects[,NCage1:Nages]
AgectsY = AgeComp; AgectsY = AgectsY[,3:(NCage1-1)]
NPRobs = nrow(df.Rep)
NPcts = nrow(df.Pup)
...
tmp = colSums(AgeComp[12:41,])
tmp = tmp[-(1:2)]
tmp2 = tmp; tmp2[1:5] = 1.2*tmp2[1:5]
tmp2 = tmp; tmp2[1:6] = 1.2*tmp2[1:6]
plot(3:36,tmp)
plot(3:36,tmp2)

# Set up Jags inputs --------------------------------------------------------
# 
fitmodel = c("HSmodfit_v3.stan")
stan.data <- list(NPcts=NPcts,NCages=NCages,NCage1=NCage1,NCobs=NCobs,
                    ...
                    Pups=Pups,sdNP=sdNP,Agects=Agects,AgectsY=AgectsY,
                    NFsamp=NFsamp,NPrg=NPrg,YrPct=YrPct,sad0=sad0,
                    YrAGsmp=YrAGsmp,YrPRsmp=YrPRsmp,AgePRsmp=AgePRsmp, 
                    omega=omega,Q_A=Q_A,Q_0=Q_0,CV_HV=CV_HV,N0pri=N0pri,
                    H_0=H_0,H_A=H_A,Snp=Snp)  
...
# Save results --------------------------------------------------------------
#
resultsfile = paste0("HS_Results_",format(Sys.time(),"%y_%b%d_%H%M"),"_agmn",NCage1,"_v3.rdata")
