...
Age_bias_adj = .2 ; # age bias adjustment: proportion reduced availability for ages <7 
#                  (0.2 = 20% reduction in sampling of age<7, 0 = 0% reduction)
Age_bias_adj_yr1 = 2001 ; # age bias adjustment applied from this year forward
#
nburnin = 300
nsamples = 5000
# End user parameters--------------------------------------------------------
...
# Age bias adjustment matri
AB_adj = matrix(1,nrow = Nyrs, ncol = NCages) 
if(NCage1<7){
  for(y in (Age_bias_adj_yr1-Yr0):Nyrs ){
    AB_adj[y,1:(7 - NCage1)] = 1-Age_bias_adj
  }
}
#
NFsamp = df.Rep$N
...
stan.data <- list(NPcts=NPcts,NCages=NCages,NCage1=NCage1,NCobs=NCobs,
                  ...
                  YrPRsmp=YrPRsmp,AgePRsmp=AgePRsmp,AB_adj=AB_adj,
...
#
fitmodel = c("HSmodfit_v1_ABadj.stan")
#
...
# Save results --------------------------------------------------------------
#
resultsfile = paste0("HS_Results_",format(Sys.time(),"%y_%b%d_%H%M"),"_agmn",NCage1,"_v1ABadj.rdata")
