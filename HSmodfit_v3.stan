...
data {
  ...
  vector[Nyrs] CI;            // array of annual environmental climate index values
  vector[Nyrs] WI;            // array of annual wind anomaly index values
  array[NCobs,NCages] int<lower=0> Agects;// matrix of adult counts by age class (cols) & year (rows)
  array[NCobs, Nages-NCages-2] int<lower=0> AgectsY;// matrix of younger counts by age class (cols) & year (rows)
  ...
  vector[NPcts] sdNP;             // s.d. associated with total pup counts
...
transformed data{
  ...
  vector[Nyrs-1] WI_CI ;
  for (i in 1:(Nyrs-1)){
    WI_CI[i] = (WI[i] - min(WI)) * (CI[i+1] - min(CI)) ;
  }
...
parameters {
  ...
  array[NCobs] simplex[Nages-NCages-2] pieY; // probs of age counts (for multinomial)
  ...
  array[2] real<lower=-10,upper=10> dlta; // Effect of environmental conditions on fecundity/pup_survival
  array[2] real<lower=-10,upper=10> psi;  // Effect of wind anomaly conditions on pup_survival 
...
transformed parameters {
  ...
  vector[Nyrs-1] Pups_pred ;              // Predicted pups available for counting, by year  
  array[Nyrs-1] vector<lower=0>[NCages] Avc;// predicted age vector
  array[Nyrs-1] vector<lower=0>[Nages-NCages-2] AvcY;// predicted age vector, younger age classes
  ...
  real tau = tau10 * 10 ;
  // Re-scale the random effect variables from standardized normal 
  epsF = epsFs * sigF ;
  epsS = epsSs * sigS ;
  ...
  for (i in 1:(Nyrs-1)){
    ...
    haz_J = exp(omega + gamma_0 + gamma_D + dlta[2] * CI[i+1] + 
                     psi[1] * WI[i] + psi[2] * WI_CI[i] + epsS[i]) ; //  juv hazards
    haz_H0 = exp(omega + gamma_H0[i]) ; // hazards from bycatch/human harvest of pups/beaters
    S0[i] = exp(-1 * (haz_J + haz_H0)) ;  // combine hazards for Juv survival
    ...
    AvcY[i] = n[i][3:(NCage1-1)] / sum( n[i][3:(NCage1-1)] ) ;
...
//  H_0 ~ gamma(square(H0_pred) ./ square(H0_pred * CV_HV), 
//                    H0_pred ./ square(H0_pred * CV_HV) ) ; 
//  H_A ~ gamma(square(HA_pred) ./ square(HA_pred * CV_HV), 
//                    HA_pred ./ square(HA_pred * CV_HV) ) ; 
  //  Annual pup counts (change to Gamma dist?)
  Pups ~ normal(Pups_pred[YrPct],sdNP) ;
//  Pups ~ gamma(square(Pups_pred[YrPct]) ./ square(sdNP),
//                      Pups_pred[YrPct] ./ square(sdNP)) ;
  ...
  for(i in 1:NCobs){
    ...
    pieY[i] ~ dirichlet( (tau/10) * AvcY[YrAGsmp[i]]) ;
    AgectsY[i,] ~ multinomial(pieY[i]) ;
  }
...
generated quantities {
  ...
  S0_ld =  exp(-1 * (exp(omega + gamma_0 + pow(phi[2]*2*.82, thta[2])) + exp(omega) )) ;
  S0_hd =  exp(-1 * (exp(omega + gamma_0 + pow(phi[2]*6*.82, thta[2])) + exp(omega) )) ;
