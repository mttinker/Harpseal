...
Agects = AgeComp
ii = which(rownames(AgeComp) >= NCyear1)
Years_ages = as.integer(rownames(AgeComp[ii,]))
NCobs = length(Years_ages)
Agects = Agects[ii,NCage1:Nages]
NPRobs = nrow(df.Rep)
NPcts = nrow(df.Pup)
...
stan.data <- list(NPcts=NPcts,NCages=NCages,NCage1=NCage1,NCobs=NCobs,
                    ...
                    Snp=Snp,Age_bias_adj=Age_bias_adj)  
...
mcmc_areas(mcmc, pars= vn[startsWith(vn,"dlta")],
  ...
  theme_classic()
#
mcmc_areas(mcmc, pars= c("alpha0","alpha1"),
           area_method="equal height",
           prob = 0.8) + 
...
resultsfile = paste0("HS_Results_",format(Sys.time(),"%y_%b%d_%H%M"),"_agmn",NCage1,
                      "_agbias",Age_bias_adj,"_agdatyr",NCyear1,"_v2b.rdata")
