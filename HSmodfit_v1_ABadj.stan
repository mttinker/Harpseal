...
data {
  ...
  matrix[Nyrs,NCages] AB_adj ;    // matrix of age bias adjustment factors for younger animals
}
...
transformed parameters {
  ...
  for (i in 1:(Nyrs-1)){
    ...
    // (including bias adjustment for younger animals)
    Avc[i] = (to_vector(AB_adj[i]) .* n[i][NCage1:Nages]) / sum((to_vector(AB_adj[i,]) .* n[i][NCage1:Nages])) ;
